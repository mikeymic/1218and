...
public class AirSwordListener implements SensorEventListener {
    ...
    private static float DETECT_ACCELERO = 13.0f;
    ...
    public AirSwordListener(Context context) {
    ...
    public void setAirSwordListener(OnAirSwordListener listener) {
	asListener = listener;
    }
	...
	List<Sensor> list = sensorManager.getSensorList(Sensor.TYPE_ACCELEROMETER);//加速度センサーを取得
	//加速度センサーが一つない場合は何もしない
	if (list.size() <1) {
	    return; 
	}
	sensorManager.registerListener(this, list.get(0), SensorManager.SENSOR_DELAY_UI);//加速度センサーの反応制度
    }
